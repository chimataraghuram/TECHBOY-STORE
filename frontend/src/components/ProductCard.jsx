...
const ProductCard = ({ product, onCompare, isComparing, onPriceAlert, index }) => {
    ...
    return (
        <>
            <m.div
                className={`group relative bg-[#0a0a0f] border border-red-500/20 rounded-2xl overflow-hidden hover:border-red-500/45 hover:shadow-[0_15px_45px_rgba(230,0,35,0.16)] shadow-[0_0_30px_rgba(255,31,61,0.06)] transition-all duration-300 flex flex-col h-full p-2.5 sm:p-3 max-w-[340px] min-[500px]:max-w-none mx-auto w-full ${isComparing ? 'ring-2 ring-red-500/60' : ''}`}
            ...
            >
                ...
                <div className="px-1 pt-2.5 pb-1 flex flex-col flex-1">
                    ...
                    <h3 className="text-white font-bold text-sm sm:text-[15px] leading-snug mt-1 line-clamp-1" title={product.name}>
                        {product.name}
                    </h3>
                    {tagline && (
                        <p className="text-gray-400 text-[11px] mt-0.5 line-clamp-1">{tagline}</p>
                    )}

                    {/* Spec chips */}
                    {specChips.length > 0 && (
                        <div className="flex items-center gap-1 mt-2 flex-wrap">
                            {specChips.slice(0, 4).map(({ icon: Icon, value }) => (
                                <div key={value} className="rounded-md border border-white/8 bg-white/[0.03] px-1.5 py-0.5 flex items-center gap-1 text-[9px] font-medium text-gray-300">
                    ...
                    <div className="flex items-baseline justify-between gap-1 mt-2.5 pt-1">
                        <div className="flex items-baseline gap-1.5">
                            <span className="text-[#ff2a3d] font-extrabold text-lg sm:text-xl tracking-tight">₹{currentPrice.toLocaleString('en-IN')}</span>
                            {discount > 0 && (
                                <span className="text-gray-600 font-semibold line-through text-[11px]">₹{prevPrice.toLocaleString('en-IN')}</span>
                            )}
                        </div>
                    ...
                    <div className="flex items-center gap-2 mt-3 pt-2.5 border-t border-white/[0.07]">
                        ...
                        <button
                            ...
                            className="relative group/btn overflow-hidden flex-1 h-[38px] rounded-xl bg-gradient-to-r from-[#ff1f3d] via-[#e60023] to-[#c7001e] hover:from-[#ff3350] hover:to-[#d6001c] text-white text-[12px] font-bold tracking-wide flex items-center justify-center gap-2 shadow-[0_4px_16px_rgba(230,0,35,0.35)] hover:shadow-[0_6px_24px_rgba(255,31,61,0.55)] transition-all active:scale-[0.97] border border-white/20"
                        >
                            ...
                            <span className="relative z-10 flex items-center justify-center w-5 h-5 rounded-full bg-white/15 transition-transform duration-300 group-hover/btn:translate-x-0.5">
                        ...
                        <m.button
                            ...
                            className="group/track relative h-[38px] w-[38px] rounded-xl bg-white/[0.04] hover:bg-red-500/15 border border-white/10 hover:border-red-500/40 text-gray-300 hover:text-red-400 flex items-center justify-center transition-all duration-200 shrink-0 hover:shadow-[0_0_14px_rgba(255,31,61,0.25)]"
                            title="Track Price"
                            aria-label="Track Price"
                        >
                            <Bell size={15} className="transition-transform duration-200 group-hover/track:scale-110" />
                        </m.button>
                        ...
                        <m.button
                            ...
                            className={`group/comp relative h-[38px] w-[38px] rounded-xl border transition-all duration-200 flex items-center justify-center shrink-0 ${
                                isComparing
                            ...
                            aria-label="Compare"
                        >
                            <ArrowLeftRight size={15} className="transition-transform duration-300 group-hover/comp:rotate-180" />
                        </m.button>
                    </div>
                </div>
            </m.div>
