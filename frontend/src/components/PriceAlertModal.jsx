...
const PriceAlertModal = ({ isOpen, onClose, product, user, triggerRect }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    setError('');
    try {
      const authedUser = await loginWithGoogle();
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const parsedTarget = alertType === 'TARGET'
        ? parseInt(String(targetPrice).replace(/\D/g, ''), 10)
        : null;

      if (alertType === 'TARGET' && !parsedTarget) {
      ...
      setError(err.message || 'Failed to set alert. Please try again.');
      console.error(err);
    } finally {
      setLoading(false);
    }
            ...
            {step === 1 && (
              ...
              >
                <div className="price-alert-header">
                  ...
                  <h3 className="price-alert-title">Sign In To Create Price Alerts</h3>
                  <p className="price-alert-desc">Create a free TECHBOY account using Google to track smartphone price changes and target price alerts.</p>
                </div>

                <div className="price-alert-auth-options">
                    <button type="button" className="price-alert-google-btn" onClick={handleGoogleLogin} disabled={loading || authLoading}>
            ...
            {step === 2 && (
              ...
              >
                <div className="price-alert-header">
                  ...
                  <p className="price-alert-desc">Choose how TECHBOY should watch {product.name} for {email}.</p>
                </div>
                ...
                <div className="price-alert-options">
                  ...
                  >
                    ...
                    <div className="price-alert-option-content">
                      <h5>Notify me whenever price changes</h5>
                  ...
                  >
                    ...
                    <div className="price-alert-option-content">
                      <h5>Notify me when price reaches my target amount</h5>
